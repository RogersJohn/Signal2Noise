          ...
          <div key={result.conspiracyId} data-testid={`result-${result.conspiracyId}`} style={styles.resultCard}>
            ...
            {result.playerResults.map(pr => {
              ...
              const reason = pr.points === 0
                ...
                : pr.hasSpecificEvidence ? '(evidence + specific)'
                : pr.hasEvidence ? '(evidence)'
                : '(bandwagon)';
              ...
              return (
                <div key={pr.playerId} data-testid={`player-score-${pr.playerId}`} style={styles.playerLine}>
                  ...
                  <span style={{ color: '#888', width: '120px', display: 'inline-block' }}>{reason}</span>
                  <span style={{ color: pr.credibilityChange > 0 ? '#0af' : '#f44', fontSize: '10px' }}>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  noResults: { color: '#888', fontSize: '12px', textAlign: 'center' },
  resultCard: { background: '#1a1a2e', borderRadius: '6px', padding: '12px', marginBottom: '12px' },
  conspiracyHeader: { fontSize: '14px', marginBottom: '8px' },
  consensusYes: { color: '#0f0' },
  consensusNo: { color: '#888' },
  playerLine: { fontSize: '12px', marginLeft: '12px', marginBottom: '3px' },
  replacedNote: { color: '#666', fontSize: '11px', marginTop: '6px', fontStyle: 'italic' },
  scoresSummary: { color: '#fa0', fontSize: '13px', textAlign: 'center', margin: '16px 0', padding: '8px', background: '#1a1a0a', borderRadius: '4px' },
  continueBtn: {
